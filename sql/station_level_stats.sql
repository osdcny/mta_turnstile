WITH sub1 AS (
    SELECT
        entries,
        exits,
        /* CAST(date AS DATE) AS date1,*/
        (date :: DATE || ' ' || time) :: timestamp AS date_time,
        TO_CHAR(date :: DATE, 'yyyy-mm') AS period,
        (station || '-' || linename) AS station1,
        (unit || '-' || "C/A" || '-' || scp) AS turnstile
    FROM
        mta.turnstile_data
    WHERE
        /*Only the three divisions that are part of the NYC subway system*/
        division IN ('BMT', 'IND', 'IRT')
        /* Only need data starting 2020; 2019 is needed to YoY calculations */
        AND date :: DATE > '2018-12-31'
        /* Filter out records that were taken randomly */
        AND (
            (time LIKE '%:00:00')
            OR (time LIKE '%:30:00')
        )
    ORDER BY
        station1,
        turnstile,
        date_time
),
turnstile_count AS (
    SELECT
        turnstile
    FROM
        sub1
        /* 
            WHERE
                date_time BETWEEN '2020-01-01' AND '2020-12-31' 
            	*/
    GROUP BY
        turnstile
    HAVING
        COUNT(turnstile) > 600
        /*Remove turnstiles that only appear occationally*/
),
sub2 AS (
    SELECT
        *,
        entries - LAG(entries, 1) OVER (PARTITION BY turnstile) AS entries_usage,
        exits - LAG(exits, 1) OVER (PARTITION BY turnstile) AS exits_usage
    FROM
        sub1
    WHERE
        turnstile IN (
            SELECT
                DISTINCT turnstile
            FROM
                turnstile_count
        )
),
result_sub AS (
    SELECT
        ref.station_name,
        period,
        SUM(abs(entries_usage)) AS entries_usage1,
        SUM(abs(exits_usage)) AS exits_usage1
    FROM
        sub2
        INNER JOIN mta.station_info AS ref ON sub2.station1 = ref.station_name_raw
    WHERE
        (entries_usage IS NOT NULL)
        AND (exits_usage IS NOT NULL)
        AND (abs(entries_usage) < 7200)
        AND (abs(exits_usage) < 7200)
    GROUP BY
        period,
        ref.station_name
    ORDER BY
        station_name,
        period
),
result_sub2 AS (
    SELECT
        *,
        LAG(entries_usage1, 12) OVER (PARTITION BY station_name) AS entries_usage1_lag,
        LAG(exits_usage1, 12) OVER (PARTITION BY station_name) AS exits_usage1_lag
    FROM
        result_sub
)
SELECT
    station_name,
    (period || '-01') AS period1,
    entries_usage1 / entries_usage1_lag AS entry_usage_yoy,
    exits_usage1 / exits_usage1_lag AS exit_usage_yoy
FROM
    result_sub2
WHERE
    (entries_usage1 IS NOT NULL)
    AND (exits_usage1 IS NOT NULL)
    AND (entries_usage1_lag IS NOT NULL)
    AND (exits_usage1_lag IS NOT NULL)
    AND (entries_usage1_lag <> 0)
    AND (exits_usage1_lag <> 0)